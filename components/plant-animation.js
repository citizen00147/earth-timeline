...
function init() {
  ...
  loader.load("plant_model-v1.glb", function (gltf) {
    gltf.scene.scale.set(0.3, 0.3, 0.3);
    let plant = gltf.scene;
    scene.add(plant);

    let box = new THREE.Box3().setFromObject(plant);
    let center = new THREE.Vector3();
    box.getCenter(center);
    plant.position.sub(center); // center the model
  ...
  const canvas = document.getElementById("displayPlant");
...
export * as plantAnimation from "/components/plant-animation.js";
