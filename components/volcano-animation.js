...
function init() {
  ...
  loader.load("volcano_model-v1.glb", function (gltf) {
    gltf.scene.scale.set(0.15, 0.15, 0.15);
    let volcano = gltf.scene;
    scene.add(volcano);

    let box = new THREE.Box3().setFromObject(volcano);
    let center = new THREE.Vector3();
    box.getCenter(center);
    volcano.position.sub(center); // center the model

    function animate() {
      requestAnimationFrame(animate);

      volcano.rotation.y += 0.002;
  ...
  const canvas = document.getElementById("displayVolcano");
...
export * as volcanoAnimation from "/components/volcano-animation.js";
