...
function init() {
  ...
  loader.load("cloud_model-v1.glb", function (gltf) {
    gltf.scene.scale.set(0.3, 0.3, 0.3);
    let cloud = gltf.scene;
    scene.add(cloud);

    let box = new THREE.Box3().setFromObject(cloud);
    let center = new THREE.Vector3();
    box.getCenter(center);
    cloud.position.sub(center); // center the model
  ...
  const canvas = document.getElementById("displayCloud");
...
export * as cloudAnimation from "/components/cloud-animation.js";
